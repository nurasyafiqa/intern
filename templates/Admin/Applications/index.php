...
?>
<!-- Skrip untuk apply datetimepicker pada input field tarikh -->
<script type="text/javascript">
$(document).ready(function() {
    // Terapkan datetimepicker pada input field tarikh
    $('input[name="application_date"], input[name="start_date"], input[name="end_date"]').datetimepicker({
        format: 'Y-m-d H:i:s',  // format yang diinginkan
        step: 30 // Langkah dalam minit
    });
});
</script>
		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars text-primary"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Application'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
	...
	<div class="table-responsive">
		...
		?>
                <tr>
                    ...
                    <th><?= $this->Paginator->sort('company_address1') ?></th>
                    <th><?= $this->Paginator->sort('company_address2') ?></th>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                    ...
                    <td><?= $application->hasValue('student') ? $this->Html->link($application->student->name, ['controller' => 'Students', 'action' => 'view', $application->student->id]) : '' ?></td>
                    <td><?= $application->hasValue('company') ? $this->Html->link($application->company->name, ['controller' => 'company', 'action' => 'view', $application->company->id]) : '' ?></td>
                    <td><?= h($this->Time->format($application->application_date, 'YYYY-MM-dd HH:mm')) ?></td>
                    <td><?= h($this->Time->format($application->start_date, 'YYYY-MM-dd HH:mm')) ?></td>
					<td><?= h($this->Time->format($application->end_date, 'YYYY-MM-dd HH:mm')) ?></td>
                    <td><?= h($application->company_name) ?></td>
                    <td><?= h($application->company_address1) ?></td>
                    <td><?= h($application->company_address2) ?></td>
                    <td><?= h($application->postcode) ?></td>
                    <td><?= h($application->city) ?></td>
                    <td><?= h($application->state) ?></td>
                    <td><?= h($application->contact_person) ?></td>
					...
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $application->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $application->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
			...
			<div class="row pb-3">
				...
				<div class="col-md-4">
				  <div class="stat_card card-2 bg-body-tertiary">
					<h3><?php echo $total_applications_active; ?></h3>
					<p>Active Applications</p>
				  </div>
				</div>
				<div class="col-md-4">
				  <div class="stat_card card-3 bg-body-tertiary">
					<h3><?php echo $total_applications_archived; ?></h3>
...
<div class="row">
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
			<div class="card-title mb-0">Applications (Monthly)</div>
...
const monthly = new Chart(ctx, {
	...
	<div class="card bg-body-tertiary border-0 shadow mb-4">
		<div class="card-body">
		<div class="card-title mb-0">Applications by Status</div>
			<div class="tricolor_line mb-3"></div>
		<div class="chart-container" style="position: relative;">
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Applications(s)',
			data: [<?= json_encode($total_applications_active); ?>, <?= json_encode($total_applications_disabled); ?>, <?= json_encode($total_applications_archived); ?>],
            backgroundColor: [
