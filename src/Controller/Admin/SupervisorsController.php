    ...
    public function view($id = null)
{
    $supervisor = $this->Supervisors->get($id, [
        'contain' => ['Employees'], // 👈 load all employees related to this supervisor
    ]);
    ...
    public function add()
    {
        ...
        $supervisors = $this->Supervisors->Supervisors->find('list', ['limit' => 200])->all();
        $this->set(compact('supervisor', 'supervisors'));
    }
	...
	public function archived($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this->Supervisors->save($supervisor)) {
                $this->Flash->success(('The supervisor has been archived.'));

				return $this->redirect($this->referer());
            }
            $this->Flash->error(('The supervisor could not be archived. Please, try again.'));
        }
        $this->set(compact('supervisor'));
    }
}